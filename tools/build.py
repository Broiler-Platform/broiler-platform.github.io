...
def prepare(out: Path) -> None:
    ...
    """
    if out.exists() and not (out / MARKER).exists():
        raise BuildError(
    ...
    # Write the marker before deleting anything, so a run interrupted halfway
    # still leaves a directory the next run recognises as its own.
    (out / MARKER).write_text(
        "Generated by tools/build.py. Everything here is rebuilt from scratch.\n",
        encoding="utf-8",
    )

    # Empty the directory rather than removing it: on Windows a server serving
    # the output holds the directory itself open, and removing the root then
    # fails even though every file inside it can be replaced.
